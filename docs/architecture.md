...
Platform entrypoints exist at two levels:

- `alethea-core/` — versioned source of truth, used when `alethea-core` is the IDE workspace root
- `alethea/` (parent root) — generated by `bootstrap.sh`, not versioned, used when the parent `alethea/` folder is the IDE workspace root

The parent-level entrypoints are identical to the `alethea-core` versions except that paths are rewritten from `agents/shared/` to `alethea-core/agents/shared/`. Bootstrap generates them on first setup and must be re-run whenever the agent family changes.
